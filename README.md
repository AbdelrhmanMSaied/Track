...
Modules 2–3, 12–14 intentionally expose narrow `SECURITY DEFINER` RPCs for atomic invite acceptance, organization bootstrap, season transition, assignment history, recruitment application, applicant acceptance, Career Passport, verification review, and Tasks Lite. The recruitment RPCs expose public campaign fields only while a campaign is live, require a completed profile to apply, prevent existing members from applying, keep profile data private except to active organization owners reviewing their applicants, and create the accepted membership in the same transaction. Career Passport RPCs keep career claims and verification requests private, require an explicit request tied to the member's own assignment snapshot, expose a request only to an active owner of that organization, and permanently prevent edits or deletion after approval. Task RPCs keep direct task/history table access revoked; active owners, board, and heads manage tasks, while active members only update progress on tasks currently assigned to them. Every exception revokes anonymous/public execution unless public campaign reading is explicitly required, validates `auth.uid()`, validates active owner/member scope where applicable, uses an empty `search_path`, and fully qualifies database objects. Supabase Advisors may report intentional authenticated-callable functions as warnings; changing them to `SECURITY INVOKER` would require broader table/profile grants.
...
Module 14 currently covers **Tasks Lite**: active-season tasks, optional department scope, one active-member assignee, due date, priority, status/progress, manager editing, assignee progress updates, and append-only server-authored history. Current task lists only show the active season; archived-season tasks remain retained with their history but are immutable in this slice. Managers may see workspace tasks; ordinary members only see tasks currently assigned to them. Projects, multi-assignee, reviewers/approvals, comments, checklists, attachments/links, dependencies, recurrence/templates, calendar/Kanban, notifications, analytics, career sync, and history-query batching remain later slices.

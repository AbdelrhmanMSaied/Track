...
Modules 2–3, 10, 12–16 and 22 intentionally expose narrow `SECURITY DEFINER` RPCs for atomic invite acceptance, organization bootstrap, season transition, assignment history, recruitment application, applicant acceptance, Career Passport, verification review, operational records, and read-only dashboards. The dashboard RPCs accept only a Cairo date range of 93 days or fewer, expose no private notes or rosters, and scope member results to their own tasks, meeting roster entries, and event registrations; recruitment totals remain owner-only. Every exception revokes anonymous/public execution unless public campaign reading is explicitly required, validates `auth.uid()`, validates active owner/member scope where applicable, uses an empty `search_path`, and fully qualifies database objects. Supabase Advisors may report intentional authenticated-callable functions as warnings; changing them to `SECURITY INVOKER` would require broader table/profile grants.
...
Module 13 currently covers public campaign links, profile-backed applications, owner review/status changes, and atomic acceptance into an active Membership. Owners may move a rejected application back to review and may accept a non-rejected applicant after a campaign closes; closing only ends new public submissions. Custom questions, interviews, assessments, notifications, analytics, and marketplace discovery remain later slices.

Module 14 currently covers **Tasks Lite**: active-season tasks, optional department scope, one active-member assignee, due date, priority, status/progress, manager editing, assignee progress updates, append-only server-authored history, and calendar surfacing for dated personal tasks. Current task lists only show the active season; archived-season tasks remain retained with their history but are immutable in this slice. Managers may see workspace tasks; ordinary members only see tasks currently assigned to them. Projects, multi-assignee, reviewers/approvals, comments, checklists, attachments/links, dependencies, recurrence/templates, Kanban, notifications, analytics, career sync, and history-query batching remain later slices.

Module 15 currently covers **Meetings & Attendance Lite**: manager-created active-season meetings, a frozen snapshot of active members at creation, manager-only present/absent/excused recording, minutes/decisions/reference URL, and action tasks linked to their source meeting. Meetings use Africa/Cairo wall time; past times remain allowed to record governance after the fact. Members see only their own attendance; archived meetings stay readable but immutable. RSVP/self-marking, recurring meetings, invitations/reminders, external calendar sync, attendance analytics, recordings beyond one HTTPS reference, projects/committees as separate entities, and career sync remain later slices.

Module 16 currently covers **Events Lite**: manager-created active-season event drafts, Cairo wall-time publishing, optional capacity, authenticated profile-complete public registration, cancellation, one active-member role per event team, and manager-only exact-roster attendance completion. Public pages expose only published event basics and capacity state; rosters, team details, and internal objectives stay private. Events block season rollover until completed or cancelled. Generic teams, volunteer applications, tasks/projects, checklists, calendar/reminders, ticketing, budgets, sponsors, evaluation, analytics, exports, notifications, marketplace discovery, and all Career Passport sync remain later slices.

Modules 10 and 22 currently cover **Calendar + Dashboards Lite**: a personal 93-day-bounded calendar/list and authenticated iCal download for assigned dated tasks, rostered scheduled meetings, and registered published events, plus role-scoped organization summary metrics. Managers see active-season operational totals; members see only their own activity; recruitment totals are owner-only. Google sync feeds, reminders, notifications, external calendar write-back, reports, charting, exports beyond iCal, department analytics, and historical reporting remain later slices.

Modules 2–3 and 32 currently cover **Career Passport + Organization Verification Lite**: a private chronological claim list, one optional HTTPS evidence URL, self-reported/evidence-provided states, and an explicit organization-verification request that snapshots one of the member's recorded assignments. Only an active owner of that snapshot's organization can review it; approval changes the claim to Organization Verified and makes it immutable. Reviewer rejection notes, an independent sensitive-action audit trail, pending-request button/loading polish, public sharing, CV export, skills/goals, automated evidence review, moderation queue, organization claiming, and platform-admin review remain later slices.
